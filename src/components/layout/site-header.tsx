        ...
        <nav className="hidden items-center gap-1 sm:flex" aria-label="Main navigation">
          {navItems.map((item) => (
            <Button
              ...
              asChild
              variant="ghost"
              className="h-9 rounded-full px-3 text-muted-foreground hover:bg-muted hover:text-primary"
            >
              <Link href={item.href}>{item.label}</Link>
            </Button>
          ))}
        </nav>
        <Button asChild className="h-9 rounded-full bg-primary px-4 text-primary-foreground shadow-sm hover:bg-primary/90">
